...
<body style="margin-top:2rem;">
    ...
    $subject = $vendeurnom . ' a fait une demande de Navette';
    $message = $vendeurnom . ' vient d\'envoyer une nouvelle demande de Navette via le formuaire <br> Lien direct vers le tableau : <a href="https://dashboard.onyos.fr/index.php">ici</a>.';
    ...
    $subjectretour = "Votre demande de Navette à bien été prise en compte !";
    $messageretour = "Nous avons bien pris en compte votre demande de Navette. <br><br>(Cette présentation demail est provisoire)<br><br> Bonne journée !";


    $str = file_get_contents('./navette-result.json');
    $arr = json_decode($str, true);
    $arrne['ARnumber'] = $_POST['ARnumber'];
    $arrne['vendeur'] = $_POST['vendeur'];
    $arrne['gamme'] = $_POST['gamme'];
    $arrne['client'] = $_POST['kunde'];
    $arrne['reference'] = $_POST['ref'];
    $arrne['week'] = $_POST['week'];
    $arrne['commentaire'] = $_POST['kommentare'];
    $arrne['date'] = $date;
    $arrne['date2'] = $date2;
    ...
    array_push($arr['navette'], $arrne);
    $str = json_encode($arr);

    if (json_decode($str) != null) {
        $file = fopen('./navette-result.json', 'w');
        fwrite($file, $str);
        fclose($file);
        echo "
